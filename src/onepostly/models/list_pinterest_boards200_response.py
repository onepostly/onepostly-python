...
class ListPinterestBoards200Response(BaseModel):
    """
    ...
    def platform_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pinterest']):
            raise ValueError("must be one of enum values ('pinterest')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in boards (list)
        _items = []
        if self.boards:
            for _item_boards in self.boards:
                _items.append(_item_boards.to_dict() if _item_boards is not None else None)
            _dict['boards'] = _items
        # set to None if default_board_id (nullable) is None
        # and model_fields_set contains the field
        if self.default_board_id is None and "default_board_id" in self.model_fields_set:
            _dict['defaultBoardId'] = None
